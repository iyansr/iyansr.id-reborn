...
export const Header = () => {
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-50 w-full">
      <DesktopHeader links={desktopLinks} asPath={asPath} />
      <MobileHeader links={mobileLinks} asPath={asPath} />
    </header>
  );
};
          ...
          <li key={label}>
            <Link
              ...
              className={`font-extrabold capitalize transition-all duration-100 ${
                isPath
                  ? 'header-link-active text-3xl text-red-custom'
                  : 'header-link text-xl text-gray-800 hover:text-red-custom'
              }`}
            >
              {label}
            </Link>
    ...
    <div className="flex w-full px-8 pt-8 md:hidden">
      ...
      {showMenu && (
        <>
          ...
          <MobileLinks asPath={asPath} links={links} setState={setShowMenu} />
        </>
      )}
    </div>
  );
